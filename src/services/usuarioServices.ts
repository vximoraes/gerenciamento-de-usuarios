...
export class Usuarios implements Usuario {
    ...
    constructor(nome: string, email: string, senha: string, papel: Papel, status: boolean) {
        this.id = uuid()
        this.nome = nome
        this.email = email
        this.senha = senha
...
cadastrarUsuario(nome: string, email: string, senha: string, papel: Papel, status: boolean) {
    if(this.papel.permCadastrar) {
        const newUser = new Usuarios(nome, email, senha, papel, status)  
